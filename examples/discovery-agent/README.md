...
- "Show me the A2A Agent Card for the k8s-agent"
- "Deploy the filesystem MCP server"

The agent uses two key MCP tools for A2A Agent Card discovery:

- `search_agent_cards` -- Search for agents that have A2A Agent Cards
- `get_agent_card` -- Retrieve the full A2A Agent Card for a specific agent

## MCP Tools Available

The agent has access to all 17 AgentRegistry MCP tools:
